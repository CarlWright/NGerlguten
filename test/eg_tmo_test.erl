...
file(File_name) ->
  Out = filename:rootname(File_name) ++ ".pdf",
...
sect1(PDF, [{code, _, Code}|T], S) ->
    ...
    sect1(PDF, T, S1);
sect1(PDF, [{list, _, List}|T], S) ->
    %% io:format("List = ~p~n",[S]),
    S1 = list1(PDF, 50, 465, List, S),
    sect1(PDF, T, S1);
...
title(PDF, Title, Level, S0) ->
    ...
    pdf:text(PDF, Num),
    TagMap = {[p], 
              [{default,mk_face("Helvetica-Bold",14,true,default,0)},
               {em,mk_face("Helvetica-Bold",14,true,default,0)},
               {code,mk_face("Courier-Bold",14,true,default,0)},
               {b,mk_face("Helvetica-Bold",14,true,default,0)}]},
    {p, _, RichText} = 
        eg_xml2richText:normalise_xml({p, [], Title}, TagMap),
...
para(PDF, Indent, Width, Para0, S) ->
    ...
    TagMap = {[p], [{default,mk_face("Times-Roman",12,true,default,0)},
                    {em,mk_face("Times-Italic",12,true,default,0)},
                    {code,mk_face("Courier",12,true,default,0)},
                    {b,mk_face("Times-Bold",12,true,default,0)}]},
    
    Norm = eg_xml2richText:normalise_xml({p, [], Para}, TagMap),
...
list1(PDF, Indent, Width, [Item|T], S) ->
    %% io:format("List = ~p~n",[Item]),
    S1 = item(PDF, Indent, Width, Item, S),
    list1(PDF, Indent, Width, T, S1);
list1(PDF, _, _, [], S) ->
    images(PDF, S).    % We did not allow pending images during the
                       % list, so we should give them a chance here.

item(PDF, Indent, Width, {item, _, Item0}, S0) ->
    ...
    pdf:set_text_pos(PDF, 80, S#st.y-16),
    pdf:text(PDF, [225]),                       % Bullet Char
    pdf:end_text(PDF),
    Item = skip_white(Item0),                   % Remove leading ws
    TagMap = {[item], [{default,mk_face("Times-Roman",12,true,default,0)},
                       {em,mk_face("Times-Italic",12,true,default,0)},
                       {code,mk_face("Courier",12,true,default,0)},
                       {b,mk_face("Times-Bold",12,true,default,0)}]},
...
code(PDF, Indent, Width, Para, S) ->
    TagMap = {[code], 
              [{default,mk_face("Courier",12,false,default,0)}]},
    %% io:format("code = ~p~n", [Para]),
...
table(PDF, Rows, X, W0, S0) ->
    ...
    Longest_words = tuple_to_list(
                      longest_words(RTF_words, erlang:make_tuple(Cols, 20000))),
...
rows(PDF, [H|T],[Row|Rows],X,Col_widths, Cols, Mid_row, S) ->
    ...
    if S#st.y - (H*12) =< S#st.min_y ->
            This_page_lines = (S#st.y - S#st.min_y) div 12,
       ...
       true ->
            ...
            rows(PDF, T, Rows, X, Col_widths, Cols, false, S1#st{y = Y - (H*12)-5})
    end;
...
row(PDF, [{Lines, Width, Off}|Cells], X, [Col_width|T], Height, Cols, S) ->
    pdf:rectangle(PDF, X,S#st.y,1,-(Height*12)-5, fill),
    pdf:begin_text(PDF),
    lines2pdf(PDF, X+3,S#st.y,Lines, 12, Width, Off, justified),
    pdf:end_text(PDF),
...
row2rtf({row, _, Row}, Col_widths) ->
    TagMap = {[cell], [{default,mk_face("Times-Roman",12,true,default,0)},
                       {em,mk_face("Times-Italic",12,true,default,0)},
                       {code,mk_face("Courier",12,true,default,0)},
                       {b,mk_face("Times-Bold",12,true,default,0)}]},
    row2rtf1(Row, Col_widths, TagMap);
row2rtf({header, _, Row}, Col_widths) ->
    TagMap = {[cell], [{default,mk_face("Times-Bold",12,true,default,0)},
                       {em,mk_face("Times-Bold",12,true,default,0)},
                       {code,mk_face("Courier-Bold",12,true,default,0)},
                       {b,mk_face("Times-Bold",12,true,default,0)}]},
    row2rtf1(Row, Col_widths, TagMap).
