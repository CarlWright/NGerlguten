%%======================================================================
%% erlguten_pdf_analyse.erl Anayse a PDF file
%%----------------------------------------------------------------------
...
tables(File) ->
    ...
    Blocks = reverse(map(fun(I) -> get_xref_entry(I, Pdf) end, Xref)),
...
get_xref_entry({Block,n,Pos,G}, Pdf) ->
    %% io:format("getting block ~p at ~p~n", [Block, Pos]),
    O = read_object(Pdf, Pos),
	...
	{I, Str1, Pos1} when integer(I)->
		...
		{J, Str2, Pos2} when integer(J) ->
			...
			{obj, Str3, Pos3} ->
			    ...
			    %% ensure(F, Pos4," endobj"),
			    %% now look after the
			    read_after_object(I, J, Obj, F, Pos4, Str4);
			Other ->
...
read_after_object(I, J, Obj, F, Pos, Str) ->
    case read_obj(Pos, F) of
	{endobj, _, _} ->
	    {{obj,I,J},Obj};
	{stream, _, Pos1}->
	    ...
	    {ok, Bin} = file:pread(F, Pos1+1, Length),
	    {{obj, I,J},{stream,Length,cvt(Bin),element(2, Obj)}}
    end.
